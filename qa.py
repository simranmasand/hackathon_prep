...
def query_folder(
    ...
    # if not return_all:
    #     sources = get_sources(result["output_text"], folder_index) TODO: Need to fix this
    print(result["output_text"])
    answer = result["output_text"].split("SOURCES: ")[0]
    print(result["output_text"].split("\nSOURCES:")) #todo; need to add the other sources as well
    source_keys = [s for s in result["output_text"].split("\nSOURCES:")[-1].split(", ")]
    print(source_keys)
    ...
    #TODO: If no sources returned, empty out the sources

    return AnswerWithSources(answer=answer, sources=sources)


def get_sources(answer: str, folder_index) -> List[Document]:
    ...
    for file in folder_index.files:
        for doc in file.docs:
            if doc.metadata["page"] in source_keys:
